...
object PaseOrderedSet {
    ...
    def getSmallestPathSetWithHighestProbability(pathSets: PathSets, probabilities: Map[Event, Probability]): (PathSet, Probability) = {
        ...
        LazyList.from(pathSetsSeq)
            ...
            .sorted(
                Ordering.by[(PathSet, Probability), Int] { case (ps, prob) => ps.size }
                    .orElse(Ordering.by[(PathSet, Probability), Probability] { case (ps, prob) => prob }.reverse)
            ).head
    }
                    ...
                    for j <- Seq(Decision.One, Decision.Zero) do
                        ...
                        if j == Decision.Zero then
                            ...
                            if Ps.contains(Set()) then
                                ...
                                val what: (PathSet, Probability) = getSmallestPathSetWithHighestProbability(Ps, basicEvents)
                                val PS: Set[Event] = what._1
//                                val maxPs: Probability = what._2
